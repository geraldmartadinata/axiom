 ...
 * Auto-growing textarea with typing animation.
 */
...
export default function CommandCapsule() {
  ...
  const overallScore = useAxiomStore(s => computeOverallScore(s.history, s.profile))
  ...
  useEffect(() => {
    if (hasUserTyped || extracting) return
    ...
    const cycleExamples = async () => {
      ...
      for (let i = 0; i <= example.length; i++) {
        if (hasUserTyped || extracting) break
        setDisplayText(example.slice(0, i))
        setInput(example.slice(0, i))
        adjustHeight()
        await new Promise(r => setTimeout(r, 30))
      }
      
      if (!hasUserTyped && !extracting) {
  ...
  }, [exampleIndex, hasUserTyped, extracting, lang])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result?.scenario) {
        useAxiomStore.getState().setCurrentScenario(result.scenario)
        navigate('/analyze')
      }
    } catch (err) {
          ...
          <div className="space-y-6">
            ...
            >
              <textarea
                ...
                placeholder={isTyping && !hasUserTyped ? '' : t('analyzer.placeholder')}
                className="w-full min-h-[100px] max-h-[200px] bg-zinc-950/50 border border-white/[6%] rounded-2xl px-5 py-4 text-white placeholder:text-zinc-600 focus:border-sand/50 focus:outline-none focus:ring-2 focus:ring-sand/20 resize-none transition-all duration-200 font-medium text-base leading-relaxed"
                style={{
                  fontFamily: 'inherit',
                  lineHeight: '1.6',
                }}
