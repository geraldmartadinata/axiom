...
export default function OpportunityCostCard({ opportunity, purchasePrice }) {
  const { t } = useLanguage()
  ...
  return (
    <Card>
      ...
      <div className="mb-6">
        ...
        <p className="text-4xl font-extrabold text-emerald-400 tracking-tighter">{formatCurrency(opportunity.total)}</p>
      ...
      <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/[6%]">
        <div>
          ...
          <p className="text-sm text-white font-medium">{formatCurrency(opportunity.futureValueDP)}</p>
        </div>
        <div>
          <p className="text-xs text-zinc-600 mb-1">Monthly Invested</p>
          <p className="text-sm text-white font-medium">{formatCurrency(opportunity.futureValueMonthly)}</p>
        </div>
      </div>
