...
export default function RecommendationCard({ sim, profile, basePrice, currency, lang }) {
  ...
  // 1) Postpone estimate: months until a 30% DP is saved from free cash flow.
  ...
  const months = fcf > 0 && gap > 0 ? Math.ceil(gap / fcf) : null
  ...
  return (
    <Card>
      ...
      <ul className="space-y-4">
        ...
        {hasIncome && months != null && (
          <li className="flex items-start gap-3">
            <CalendarClock className="h-4 w-4 mt-0.5 text-amber-400 shrink-0" />
            <div>
              <p className="text-sm text-white font-medium">{t('analyze.reco.postpone').replace('{months}', months)}</p>
              <p className="text-xs text-zinc-500 mt-0.5">
                {t('analyze.reco.postponeTarget').replace('{amount}', formatCurrency(targetDP, uiLang, currency))}
                {' · '}
                {t('analyze.reco.postponeFcf').replace('{amount}', formatCurrency(fcf, uiLang, currency))}
              </p>
            </div>
          </li>
        )}
