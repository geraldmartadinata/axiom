...
/**
 * /analyze/:sessionId — the full per-session workspace.
 * Pure function of sessionId: score, breakdown, cards, live parameters,
 * projections, and the confirm-purchase hook.
 */
export default function AnalyzeSession() {
  ...
  if (!session) {
    return (
      <div className="pt-28 pb-16 max-w-2xl mx-auto px-4 sm:px-6 text-center">
        <h1 className="font-display text-2xl font-bold text-white mb-2">{t('analyze.sessionNotFound')}</h1>
        <p className="text-sm text-zinc-500 mb-8">{t('analyze.sessionNotFoundSub')}</p>
        <Link
          to="/analyze"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold text-white hover:bg-white/10 transition-all"
        >
  ...
  const scoreColor =
    ...
    score.score >= 50 ? 'text-amber-400' : 'text-red-400'
  ...
  return (
    <div className="pt-28 pb-16 max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div className="flex items-center gap-4">
          <Link
            ...
            aria-label={t('analyze.backToAnalyses')}
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-white tracking-tight">{session.scenario?.item_name}</h1>
        ...
        <div className="flex items-center gap-2">
          {confirmed ? (
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-600 text-zinc-950 text-sm font-bold shadow-[0_0_20px_rgba(16,185,129,0.25)] hover:shadow-[0_0_32px_rgba(16,185,129,0.4)] hover:-translate-y-px transition-all"
            >
              <CheckCircle2 className="h-4 w-4" />
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-zinc-500 hover:text-red-400 hover:border-red-500/30 transition-all"
            aria-label={t('analyze.deleteSession')}
          >
      ...
      {/* Projections v2 */}
      <div className="rounded-3xl border border-white/[6%] bg-zinc-900/60 backdrop-blur-xl p-6 sm:p-8">
        <h2 className="font-display text-lg font-semibold text-white mb-1">{t('projections.title')}</h2>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        <div className="p-6">
          <h3 className="font-display text-lg font-semibold text-white mb-1">{t('analyze.confirmModalTitle')}</h3>
